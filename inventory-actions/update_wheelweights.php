...
$stmt = $conn->prepare("SELECT product_id FROM wheelweights_details WHERE wheel_id = ?");
$stmt->bind_param("i", $wheelId);
$stmt->execute();
...
$stmt = $conn->prepare("
    SELECT 
        ...
        i.quantity,
        w.model, w.weight, w.material
    FROM products p
    JOIN inventory i ON p.product_id = i.product_id
    JOIN wheelweights_details w ON p.product_id = w.product_id
...
try {
    $stmt1 = $conn->prepare("UPDATE wheelweights_details SET model = ?, weight = ?, material = ? WHERE wheel_id = ?");
    $stmt1->bind_param("sisi", $model, $weight, $material, $wheelId);
    $stmt1->execute();
    $stmt1->close();
    ...
    if ($old['weight'] != $weight) $changes[] = "Weight {$old['weight']} → $weight";
