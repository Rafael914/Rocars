...
$motortireId   = (int)($_POST['motortire_id'] ?? 0);
...
$stmt = $conn->prepare("SELECT product_id FROM motorcycle_tires_details WHERE motortire_id = ?");
$stmt->bind_param("i", $motortireId);
$stmt->execute();
...
$stmt = $conn->prepare("
    SELECT 
        ...
        i.quantity,
        m.brand, m.model, m.type, m.size
    FROM products p
    JOIN inventory i ON p.product_id = i.product_id
    JOIN motorcycle_tires_details m ON p.product_id = m.product_id
...
try {
    $stmt1 = $conn->prepare("UPDATE motorcycle_tires_details SET brand = ?, model = ?, type = ?, size = ? WHERE motortire_id = ?");
    $stmt1->bind_param("sssii", $brand, $model, $type, $size, $motortireId);
    $stmt1->execute();
    $stmt1->close();
