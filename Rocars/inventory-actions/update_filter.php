...
$brand         = $_POST['brand'] ?? '';
$typeoffilter  = $_POST['typeoffilter'] ?? '';

$branchId = $_SESSION['branch_id'] ?? 1;
$userId   = $_SESSION['user_id'] ?? 0;

$stmt = $conn->prepare("SELECT product_id FROM filter_details WHERE filter_id = ?");
...
$stmt = $conn->prepare("
    SELECT 
        ...
        i.quantity,
        f.brand, f.typeoffilter
    FROM products p
    JOIN inventory i ON p.product_id = i.product_id AND i.branch_id = ?
    JOIN filter_details f ON p.product_id = f.product_id
    WHERE p.product_id = ?
");
...
try {
    $stmt1 = $conn->prepare("UPDATE filter_details SET brand = ?, typeoffilter = ? WHERE filter_id = ?");
    $stmt1->bind_param("ssi", $brand, $typeoffilter, $filterId);
    $stmt1->execute();
    $stmt1->close();
    ...
    if ($old['typeoffilter'] != $typeoffilter) $changes[] = "TypeOfFilter '{$old['typeoffilter']}' → '$typeoffilter'";
    ...
    if (!empty($changes)) {
        $logMessage = "Updated filter '$productName': " . implode(', ', $changes);
