...
$lugnutId       = (int)($_POST['lugnut_id'] ?? 0);
$typeoflugnut   = $_POST['typeoflugnut'] ?? '';
...
$stmt = $conn->prepare("SELECT product_id FROM lugnuts_details WHERE lugnut_id = ?");
$stmt->bind_param("i", $lugnutId);
$stmt->execute();
...
$stmt = $conn->prepare("
    SELECT 
        ...
        i.quantity,
        l.typeoflugnut, l.size
    FROM products p
    JOIN inventory i ON p.product_id = i.product_id
    JOIN lugnuts_details l ON p.product_id = l.product_id
...
try {
    $stmt1 = $conn->prepare("UPDATE lugnuts_details SET typeoflugnut = ?, size = ? WHERE lugnut_id = ?");
    $stmt1->bind_param("sdi", $typeoflugnut, $size, $lugnutId);
    $stmt1->execute();
    $stmt1->close();
    ...
    $changes = [];
    if ($old['typeoflugnut'] != $typeoflugnut) $changes[] = "TypeOfLugnut '{$old['typeoflugnut']}' → '$typeoflugnut'";
